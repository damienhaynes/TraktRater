namespace TraktRater
{
  ...
  public partial class TraktRater : Form
  {
    ...
    private void StartImport()
    {
      ...
      if ( AppSettings.EnableTMDb )
        mSites.Add( new TMDb( AppSettings.TMDbRequestToken, AppSettings.TMDbSessionId ) );
      if ( AppSettings.EnableTVDb )
        mSites.Add( new TVDb( AppSettings.TVDbAccountIdentifier ) );
      if ( AppSettings.EnableIMDb )
        mSites.Add( new IMDb( AppSettings.IMDbRatingsFilename, AppSettings.IMDbWatchlistFilename, AppSettings.IMDbCustomLists, rdnImdbCSV.Checked ) );
      if ( AppSettings.EnableIMDb )
        mSites.Add( new IMDbWeb( AppSettings.IMDbUsername, rdnImdbUsername.Checked ) );
      if ( AppSettings.EnableListal )
        mSites.Add( new Listal( AppSettings.ListalMovieFilename, AppSettings.ListalShowFilename, AppSettings.ListalSyncWatchlist ) );
      if ( AppSettings.EnableCriticker )
        mSites.Add( new Criticker( AppSettings.CritickerCSVFilename ) );
      if ( AppSettings.EnableLetterboxd )
        mSites.Add( new Letterboxd( AppSettings.LetterboxdRatingsFilename, AppSettings.LetterboxdWatchedFilename, AppSettings.LetterboxdWatchlistFilename, AppSettings.LetterboxdDiaryFilename, AppSettings.LetterboxdCustomLists ) );
      if ( AppSettings.EnableFlixster )
        mSites.Add( new Flixster( AppSettings.FlixsterUserId, AppSettings.FlixsterSyncWantToSee ) );
      if ( AppSettings.EnableCheckMovies )
        mSites.Add( new CheckMovies( AppSettings.CheckMoviesFilename, AppSettings.CheckMoviesDelimiter ) );
      if ( AppSettings.EnableToDoMovies )
        mSites.Add( new ToDoMovies( AppSettings.ToDoMovieFilename ) );
      if ( AppSettings.EnableMovieLens )
        mSites.Add( new MovieLens( AppSettings.MovieLensRatingsFilename, AppSettings.MovieLensActivityFilename, AppSettings.MovieLensTagsFilename, AppSettings.MovieLensWishListFilename ) );
